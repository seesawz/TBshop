        ...
        {shopList.map((item, idx) => (
          <div onClick={() => toShopDetail(item)} key={idx} className={styles.shoplist}>
            <img
              ...
              src={`http://43.139.230.109:9002/img/${item.spuImgUrl
                .split('/')
                .at(-1)}`}
              alt=""
            />
            ...
            <p
              ...
              style={{ marginTop: '40px', fontSize: '12px', wordBreak: 'break-all' }}
            >
              {item.spuDescription}
            </p>
            <p className={styles.shopprice}>¥{item.price}</p>
            {item.top && (
              <Tag className="absolute bottom-1 right-0" color="#f50">
                Hot
              </Tag>
            )}
          </div>
